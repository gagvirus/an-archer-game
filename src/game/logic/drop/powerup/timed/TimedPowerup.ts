...
abstract class TimedPowerup extends Powerup {
  ...
  protected constructor(scene: Scene, x: number, y: number, name: string) {
    ...
    (this.scene as MainScene).events.on("GamePaused", () => {
      if (this.collectedAt) {
        this.remainingDurationSeconds = (Date.now() - this.collectedAt) / 1000;
        clearTimeout(this.timeout);
      }
    }).on("GameResumed", () => {
      if (this.remainingDurationSeconds) {
        this.timeout = this.setPowerupTimeout(scene as MainScene, this.remainingDurationSeconds * 1000);
      }
    });
  }
}
