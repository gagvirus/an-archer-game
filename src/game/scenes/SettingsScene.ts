...
class SettingsScene extends Scene implements ISceneLifecycle {
  ...
  private addSettingsRow(
    ...
    label: string,
    textOffset: Vector2Like,
    checkboxOffset: Vector2Like,
  ) {
    const checkbox: Rectangle = this.add
      .rectangle(
        checkboxOffset.x,
        checkboxOffset.y,
        20,
        20,
        this[settingKey] ? HEX_COLOR_SUCCESS : HEX_COLOR_DANGER,
      )
      .setInteractive();
